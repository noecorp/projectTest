 ...
 * @author Opiks
 */
public class SinarMasPayment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "SinarMasVAFullPayment" + "_" + request.getParameter("MALLID") + "_" + request.getParameter("VAID");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest Receiving Payment Request from MALLID=" + request.getParameter("MALLID") + ", VAID=" + request.getParameter("VAID") + ", STEP=" + request.getParameter("STEP"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.SinarMasVAFull);
            if (onecheckout.getMessage().equals("VALID")) {
                OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest Parameters Valid");
                InterfaceOneCheckoutV1BeanLocal proc = OneCheckoutServiceLocator.lookupLocal(InterfaceOneCheckoutV1BeanLocal.class);
                OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest SinarMas Step :" + STEP);
                if (STEP != null && (STEP.equalsIgnoreCase("REVERSE") || STEP.equalsIgnoreCase("REVERSAL"))) {
                    OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest REVERSAL");
                    onecheckout = proc.ProcessReversal(onecheckout);
                } else if (STEP != null && STEP.equalsIgnoreCase("SETTLEMENT")) {
                    OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest SETTLEMENT");
                    onecheckout = proc.ProcessReconcile(onecheckout);
                } else {
                    OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest PAYMENT");
                    onecheckout = proc.ProcessInvokeStatus(onecheckout);
                }
                ack = onecheckout.getNotifyRequest().getACKNOWLEDGE();
            }
            OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);
        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("SinarMasVAFullPayment.processRequest ACKNOWLEDGE : STOP");
        } finally {
            out.close();
            params = null;
            logCode = null;
