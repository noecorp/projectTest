 ...
 */
public class KlikBCAInvoke extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "KlikBCAInvoke" + "_" + request.getParameter("transactionNo");
            WebUtils.setSecurityAssociation(logCode);                
            OneCheckoutLogger.log("KlikBCAInvoke.processRequest Receiving Notify Request from trasactionNo=" + request.getParameter("trasactionNo") + ", userId=" + request.getParameter("userId"));            
            
            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.KlikBCA);            
            
            
            if (onecheckout.getMessage().equals("VALID")) {   
                
                OneCheckoutLogger.log("KlikBCAInvoke.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("KlikBCAInvoke.processRequest ACKNOWLEDGE : %s",ack);
            out.print(ack);
           
        } catch (Throwable th) { 
            out.print("STOP");
            OneCheckoutLogger.log("KlikBCAInvoke.processRequest ACKNOWLEDGE : STOP");
