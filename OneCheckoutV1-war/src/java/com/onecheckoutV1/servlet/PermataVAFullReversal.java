 ...
 * @author aditya
 */
public class PermataVAFullReversal extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "PermataVAFullReversal" + "_" + request.getParameter("VAID");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("PermataVAFullReversal.processRequest Receiving Reversal Request from VAID = " + request.getParameter("VAID") + ", USERACCOUNT=" + request.getParameter("USERACCOUNT"));

            onecheckout = WebUtils.parseReversalDOKURequestData(params, request, session, OneCheckoutPaymentChannel.PermataVAFull);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("PermataVAFullReversal.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("PermataVAFullReversal.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("PermataVAFullReversal.processRequest ACKNOWLEDGE : STOP");
