 ...
 * @author Opiks
 */
public class SinarMasReversal extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "SinarMasVAFullReversal" + "_" + request.getParameter("VAID");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("SinarMasVAFullReversal.processRequest Receiving Reversal Request from VAID = " + request.getParameter("VAID") + ", USERACCOUNT=" + request.getParameter("USERACCOUNT"));
            onecheckout = WebUtils.parseReversalDOKURequestData(params, request, session, OneCheckoutPaymentChannel.SinarMasVAFull);
            if (onecheckout.getMessage().equals("VALID")) {
                OneCheckoutLogger.log("SinarMasVAFullReversal.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("SinarMasVAFullReversal.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);
        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("SinarMasVAFullReversal.processRequest ACKNOWLEDGE : STOP");
