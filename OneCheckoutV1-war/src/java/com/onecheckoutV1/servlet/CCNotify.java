 ...
 */
public class CCNotify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "CCNotify" + "_" + request.getParameter("OrderNumber");
            WebUtils.setSecurityAssociation(logCode);            
            OneCheckoutLogger.log("CCNotify.processRequest Receiving Notify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", OrderNumber=" + request.getParameter("OrderNumber"));            

            channel = WebUtils.paymentChannel(request.getParameter("OrderNumber"), request.getParameter("SESSIONID"), request.getParameter("AMOUNT"), OneCheckoutTransactionState.NOTIFYING);

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, channel);
            
            if (onecheckout.getMessage().equals("VALID")) {   
                
                OneCheckoutLogger.log("CCNotify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("CCNotify.processRequest ACKNOWLEDGE : %s",ack);
            out.print(ack);
           
        } catch (Throwable th) { 
            out.print("STOP");
            OneCheckoutLogger.log("CCNotify.processRequest ACKNOWLEDGE : STOP");
