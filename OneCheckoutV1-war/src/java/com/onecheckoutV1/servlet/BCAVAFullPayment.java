 ...
 * @author margono
 */
public class BCAVAFullPayment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "BCAVAFullPayment" + "_" + request.getParameter("MALLID")+ "_" + request.getParameter("VANUMBER");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("BCAVAFullPayment.processRequest Receiving Payment Request from MALLID=" + request.getParameter("MALLID") + ", VANUMBER=" + request.getParameter("VANUMBER") + ", STEP=" + request.getParameter("STEP"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.BCAVAFull);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("BCAVAFullPayment.processRequest Parameters Valid");

                InterfaceOneCheckoutV1BeanLocal proc = OneCheckoutServiceLocator.lookupLocal(InterfaceOneCheckoutV1BeanLocal.class);

                OneCheckoutLogger.log("BCAVAFullPayment.processRequest Permata Step :" + STEP);

                if(STEP != null && STEP.equalsIgnoreCase("REVERSE")) {

                    OneCheckoutLogger.log("BCAVAFullPayment.processRequest REVERSAL");

                    onecheckout = proc.ProcessReversal(onecheckout);

                } else if (STEP != null && STEP.equalsIgnoreCase("SETTLEMENT")) {

                    OneCheckoutLogger.log("BCAVAFullPayment.processRequest SETTLEMENT");

                    onecheckout = proc.ProcessReconcile(onecheckout);

                } else {

                    OneCheckoutLogger.log("BCAVAFullPayment.processRequest PAYMENT");
            ...
            OneCheckoutLogger.log("BCAVAFullPayment.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("BCAVAFullPayment.processRequest ACKNOWLEDGE : STOP");
