 ...
 * @author aditya
 */
public class BRIPayVerify extends HttpServlet {
@Override
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "BRIPayVerify" + "_" + request.getParameter("TRANSIDMERCHANT");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("BRIPayVerify.processRequest Receiving Verify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", TRANSIDMERCHANT=" + request.getParameter("TRANSIDMERCHANT"));


            onecheckout = WebUtils.parseVerifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.BRIPay);


            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("BRIPayVerify.processRequest Parameters Valid");
            ...
            out.print(ack);
            OneCheckoutLogger.log("BRIPayVerify.processRequest ACKNOWLEDGE : %s",ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("BRIPayVerify.processRequest ACKNOWLEDGE : STOP");
