...
public class CaptureRequest extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "CaptureReq" + "_" + request.getParameter("TRANSIDMERCHANT");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("CaptureReq.processRequest Receiving Void Request from SESSIONID=" + request.getParameter("SESSIONID") + ", TRANSIDMERCHANT=" + request.getParameter("TRANSIDMERCHANT"));

            onecheckout = WebUtils.parseVoidRequestData(params, request, session);

            if (onecheckout.getMessage().equals("VALID")) {
                OneCheckoutLogger.log("CaptureReq.processRequest Parameters Valid");
            ...
            out.print(ack);
            OneCheckoutLogger.log("CaptureReq.processRequest ACKNOWLEDGE : %s", ack);
        } catch (Throwable th) {
            out.print("FAILED:Internal Error");
            OneCheckoutLogger.log("CaptureReq.processRequest ACKNOWLEDGE : FAILED");
        } finally {
            out.close();
            params = null;
            logCode = null;
