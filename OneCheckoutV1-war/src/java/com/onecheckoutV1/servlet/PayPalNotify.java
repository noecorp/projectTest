...
public class PayPalNotify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "PayPalNotify" + "_" + request.getParameter("OrderNumber");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("PayPalNotify.processRequest Receiving Notify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", OrderNumber=" + request.getParameter("OrderNumber"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.PayPal);


            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("PayPalNotify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("PayPalNotify.processRequest ACKNOWLEDGE : %s",ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("PayPalNotify.processRequest ACKNOWLEDGE : STOP");
