 ...
 */
public class PTPPOSPayment extends HttpServlet {
...
String STEPParam = "STEP";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "PTPOSPayment" + "_" + request.getParameter("MALLID")+ "_" + request.getParameter("VANUMBER");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("PTPOSPayment.processRequest Receiving Payment Request from MALLID=" + request.getParameter("MALLID") + ", VANUMBER=" + request.getParameter("VANUMBER") + ", STEP=" + request.getParameter("STEP"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.PTPOS);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("PTPOSPayment.processRequest Parameters Valid");

                InterfaceOneCheckoutV1BeanLocal proc = OneCheckoutServiceLocator.lookupLocal(InterfaceOneCheckoutV1BeanLocal.class);

                OneCheckoutLogger.log("PTPOSPayment.processRequest Permata Step :" + STEP);

                if(STEP != null && STEP.equalsIgnoreCase("REVERSE")) {

                    OneCheckoutLogger.log("PTPOSPayment.processRequest REVERSAL");

                    onecheckout = proc.ProcessReversal(onecheckout);

                } else if (STEP != null && STEP.equalsIgnoreCase("SETTLEMENT")) {

                    OneCheckoutLogger.log("PTPOSPayment.processRequest SETTLEMENT");

                    onecheckout = proc.ProcessReconcile(onecheckout);

                } else {

                    OneCheckoutLogger.log("PTPOSPayment.processRequest PAYMENT");
            ...
            OneCheckoutLogger.log("PTPOSPayment.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("PTPOSPayment.processRequest ACKNOWLEDGE : STOP");
