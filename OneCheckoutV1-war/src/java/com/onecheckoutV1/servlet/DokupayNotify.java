 ...
 * @author aditya
 */
public class DokupayNotify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "DokupayNotify" + "_" + request.getParameter("OrderNumber");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("DokupayNotify.processRequest Receiving Notify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", OrderNumber=" + request.getParameter("OrderNumber"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session,  OneCheckoutPaymentChannel.DOKUPAY);


            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("DokupayNotify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("DokupayNotify.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("DokupayNotify.processRequest ACKNOWLEDGE : STOP");
