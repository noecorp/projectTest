 ...
 * @author aditya
 */
public class DokupayVerify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "DokupayVerify" + "_" + request.getParameter("TRANSIDMERCHANT");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("DokupayVerify.processRequest Receiving Verify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", TRANSIDMERCHANT=" + request.getParameter("TRANSIDMERCHANT"));


            onecheckout = WebUtils.parseVerifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.DOKUPAY);


            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("DokupayVerify.processRequest Parameters Valid");
            ...
            out.print(ack);
            OneCheckoutLogger.log("DokupayVerify.processRequest ACKNOWLEDGE : %s",ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("DokupayVerify.processRequest ACKNOWLEDGE : STOP");
