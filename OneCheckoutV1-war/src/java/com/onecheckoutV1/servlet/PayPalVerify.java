 ...
 * @author aditya
 */
public class PayPalVerify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "PayPalVerify" + "_" + request.getParameter("TRANSIDMERCHANT");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("PayPalVerify.processRequest Receiving Verify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", TRANSIDMERCHANT=" + request.getParameter("TRANSIDMERCHANT"));

            onecheckout = WebUtils.parseVerifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.PayPal);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("PayPalVerify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("PayPalVerify.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("PayPalVerify.processRequest ACKNOWLEDGE : STOP");
