 ...
 * @author aditya
 */
public class MandiriClickPayNotify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "MandiriClickPayNotify" + "_" + request.getParameter("OrderNumber");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("MandiriClickPayNotify.processRequest Receiving Notify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", OrderNumber=" + request.getParameter("OrderNumber"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session,  OneCheckoutPaymentChannel.DirectDebitMandiri);


            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("MandiriClickPayNotify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("MandiriClickPayNotify.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("MandiriClickPayNotify.processRequest ACKNOWLEDGE : STOP");
