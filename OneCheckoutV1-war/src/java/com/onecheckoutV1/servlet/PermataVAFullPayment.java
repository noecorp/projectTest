 ...
 * @author aditya
 */
public class PermataVAFullPayment extends HttpServlet {
String STEPParam = "STEP";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "PermataVAFullPayment" + "_" + request.getParameter("MALLID")+ "_" + request.getParameter("VANUMBER");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("PermataVAFullPayment.processRequest Receiving Payment Request from MALLID=" + request.getParameter("MALLID") + ", VANUMBER=" + request.getParameter("VANUMBER") + ", STEP=" + request.getParameter("STEP"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.PermataVAFull);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("PermataVAFullPayment.processRequest Parameters Valid");

                InterfaceOneCheckoutV1BeanLocal proc = OneCheckoutServiceLocator.lookupLocal(InterfaceOneCheckoutV1BeanLocal.class);

                OneCheckoutLogger.log("PermataVAFullPayment.processRequest Permata Step :" + STEP);
                ...
                if(STEP != null && STEP.equalsIgnoreCase("REVERSE")) {
                    ...
                    OneCheckoutLogger.log("PermataVAFullPayment.processRequest SETTLEMENT");

                    onecheckout = proc.ProcessReconcile(onecheckout);

                } else {

                    OneCheckoutLogger.log("PermataVAFullPayment.processRequest PAYMENT");
            ...
            OneCheckoutLogger.log("PermataVAFullPayment.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("PermataVAFullPayment.processRequest ACKNOWLEDGE : STOP");
