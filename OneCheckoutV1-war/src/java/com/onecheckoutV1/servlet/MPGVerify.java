 ...
 * @author aditya
 */
public class MPGVerify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "MPGVerify" + "_" + request.getParameter("INVOICENUMBER");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("MPGVerify.processRequest Receiving Verify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", TRANSIDMERCHANT=" + request.getParameter("INVOICENUMBER"));

            onecheckout = WebUtils.parseVerifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.BSP);


            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("MPGVerify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("MPGVerify.processRequest ACKNOWLEDGE : %s", ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("MPGVerify.processRequest ACKNOWLEDGE : STOP");
