 ...
 * @author aditya
 */
public class MPGNotify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "MPGNotify" + "_" + request.getParameter("INVOICENUMBER");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("MPGNotify.processRequest Receiving Notify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", OrderNumber=" + request.getParameter("INVOICENUMBER"));

            onecheckout = WebUtils.parseNotifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.BSP);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("MPGNotify.processRequest Parameters Valid");
            ...
            OneCheckoutLogger.log("MPGNotify.processRequest ACKNOWLEDGE : %s",ack);
            out.print(ack);

        } catch (Throwable th) {
            th.printStackTrace();
            out.print("STOP");
            OneCheckoutLogger.log("MPGNotify.processRequest ACKNOWLEDGE : STOP");
