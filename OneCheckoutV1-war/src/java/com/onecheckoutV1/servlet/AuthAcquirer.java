 ...
 * @author Opiks
 */
public class AuthAcquirer extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, TemplateException {
        ...
        try {
            ...
            OneCheckoutDataHelper oneCheckoutDataHelper = WebUtils.parseAcquirerData(params, request, session);
            if (oneCheckoutDataHelper.getMessage().equals("REDIRECTACQUIRER")) {
                session.setAttribute(sessionCIPRequest, oneCheckoutDataHelper);
            } else {
                OneCheckoutLogger.log("AuthAcquirer.processRequest ERROR : %s", oneCheckoutDataHelper.getMessage());
                ...
                OneCheckoutLogger.log("AuthAcquirer.processRequest parseACSData NOT VALID");
                return;
            }
            logCode = "AuthAcquirer_" + oneCheckoutDataHelper.getMallId() + "_" + oneCheckoutDataHelper.getTransactions().getIncTransidmerchant();
    ...
    public void proses(OneCheckoutDataHelper onecheckout, HttpServletRequest request, PrintWriter out) {
        try {
            ...
            String actionUrl = config.getString("PAYMENT3DS.ACTION.URL", "/Suite/ProcessAcqPayment");
