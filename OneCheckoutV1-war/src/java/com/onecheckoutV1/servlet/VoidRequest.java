 ...
 * @author aditya
 */
public class VoidRequest extends HttpServlet {
@Override
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String ack = "FAILED;Invalid Parameter";
        ...
        try {
            ...
            logCode = "VoidReq" + "_" + request.getParameter("TRANSIDMERCHANT");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("VoidReq.processRequest Receiving Void Request from SESSIONID=" + request.getParameter("SESSIONID") + ", TRANSIDMERCHANT=" + request.getParameter("TRANSIDMERCHANT"));

            onecheckout = WebUtils.parseVoidRequestData(params, request, session);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("VoidReq.processRequest Parameters Valid");

                InterfaceOneCheckoutV1BeanLocal proc = OneCheckoutServiceLocator.lookupLocal(InterfaceOneCheckoutV1BeanLocal.class);

                onecheckout = proc.ProcessVoid(onecheckout);

                ack = onecheckout.getVoidRequest().getVOIDRESPONSE();

            }

            out.print(ack);
            OneCheckoutLogger.log("VoidReq.processRequest ACKNOWLEDGE : %s", ack);

        } catch (Throwable th) {
            out.print("FAILED:Internal Error");
            OneCheckoutLogger.log("VoidReq.processRequest ACKNOWLEDGE : FAILED");
        } finally {
            out.close();
            params = null;
            logCode = null;
