 ...
 * @author aditya
 */
public class TokenVoid extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            logCode = "CCVoid" + "_" + request.getParameter("OrderNumber");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("CCVoid.processRequest Receiving Notify Request from SESSIONID=" + request.getParameter("SESSIONID") + ", OrderNumber=" + request.getParameter("OrderNumber"));

            channel = WebUtils.paymentChannel(request.getParameter("TRANSIDMERCHANT"), request.getParameter("SESSIONID"), request.getParameter("AMOUNT"), OneCheckoutTransactionState.DONE);

            onecheckout = WebUtils.parseCCRequestData(params, request, session, channel);

            if (onecheckout.getMessage().equals("VALID")) {

                OneCheckoutLogger.log("CCVoid.processRequest Parameters Valid");

                InterfaceOneCheckoutV1BeanLocal proc = OneCheckoutServiceLocator.lookupLocal(InterfaceOneCheckoutV1BeanLocal.class);

                onecheckout = proc.ProcessCCVoid(onecheckout);

                ack = onecheckout.getNotifyRequest().getACKNOWLEDGE();

            }

            OneCheckoutLogger.log("CCVoid.processRequest ACKNOWLEDGE : %s",ack);
            out.print(ack);

        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("CCVoid.processRequest ACKNOWLEDGE : STOP");
