 ...
 * @author Opiks
 */
public class SinarMasInquiry extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logCode = "SinarMasVAInquiry" + "_" + request.getParameter("MALLID") + "_" + request.getParameter("VAID");
            WebUtils.setSecurityAssociation(logCode);
            OneCheckoutLogger.log("SinarMasVAInquiry.processRequest Receiving Verify Request from merchantCode=" + request.getParameter("MALLID") + ", VAID=" + request.getParameter("VAID"));
            onecheckout = WebUtils.parseVerifyDOKURequestData(params, request, session, OneCheckoutPaymentChannel.SinarMasVAFull);
            if (onecheckout.getMessage().equals("VALID")) {
                OneCheckoutLogger.log("SinarMasVAInquiry.processRequest Parameters Valid");
            ...
            System.out.println("SinarMasVAInquiry.processRequest ACKNOWLEDGE : "+ack);
            out.print(ack);
        } catch (Throwable th) {
            out.print("STOP");
            OneCheckoutLogger.log("SinarMasVAInquiry.processRequest ACKNOWLEDGE : STOP");
