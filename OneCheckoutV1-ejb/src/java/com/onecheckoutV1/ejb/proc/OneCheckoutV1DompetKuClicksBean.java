 ...
 */
@Stateless
public class OneCheckoutV1DompetKuClicksBean extends OneCheckoutChannelBase implements OneCheckoutV1DompetKuClicksBeanLocal{
    ...
    private final static String mIDoAcquirerId;
    private final static String serviceId;
    
    static{
        mIDoAcquirerId = config.getString("ACQUIRERID.DOMPETKU.VALUE","1008").trim();
    ...
    public OneCheckoutDataHelper doPayment(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        ...
        try {
            ...
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doPayment - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            if (request_good) {
                ...
                PaymentChannel pc = queryHelper.getPaymentChannelByChannel(OneCheckoutPaymentChannel.DompetKu);
                ...
                if (trxHelper.getCIPMIP() == OneCheckoutMethod.MIP) {
                    ...
                    OneCheckoutLogger.log("URL MIB     : "+CoreUrl);
                    
                    String xmls = sendMIBRequest(trxHelper.getPaymentRequest(), CoreUrl, mpc, m,trans);                          
                    if (xmls != null && !xmls.equals("")){
                        XMLConfiguration xml = new XMLConfiguration();
                        try {
                            StringReader sr = new StringReader(xmls);
                            xml.load(sr);
                            if (xml.getRootElementName().equals("MIBPaymentResponse")){
                                
                                String resultResp = xml.getString("result") != null ? xml.getString("result").trim() : "";    
                                OneCheckoutLogger.log("STATUS RESP " + resultResp);
                                try {
                                    if (resultResp != null){
                                        trans = this.parsingMIBDompetKuData(trans, xml);
                                     ...
                                     OneCheckoutLogger.log("::: FAILED READING RESPONSE :::");
                        ...
                        OneCheckoutLogger.log("::: FAILED GETTING RESPONSE FROM MIB :::");
            ...
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doPayment - T2: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
    ...
    public OneCheckoutDataHelper doInvokeStatus(OneCheckoutDataHelper trxHelper, PaymentChannel paymentChannel) {
          try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus - T0: %d Start process", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus - T1: %d querying transaction", (System.currentTimeMillis() - t1));
            Transactions trans = queryHelper.getNotifyTransactionBy(invoiceNo, sessionId, paymentChannel);
            
            if (trans == null) {
                OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus : Transaction is null");
            ...
            if (!word.equalsIgnoreCase(notifyRequest.getWORDS())) {
                OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus : WORDS doesn't match !");
                notifyRequest.setACKNOWLEDGE(OneCheckoutVerifyFormatData.NOTIFYSTATUS_STOP);
                trxHelper.setNotifyRequest(notifyRequest);
                return trxHelper;
            }
            
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus - T2: %d update transaction", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus - T3: %d start Notify to Merchant", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus - T4: %d update trx record", (System.currentTimeMillis() - t1));
            em.merge(trans);
            
            notifyRequest.setACKNOWLEDGE(resp);
            trxHelper.setNotifyRequest(notifyRequest);
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doInvokeStatus - T5: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
    ...
    public OneCheckoutDataHelper doRedirectToMerchant(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
     try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doRedirectToMerchant - T0: %d Start process", (System.currentTimeMillis() - t1));
            Merchants m = trxHelper.getMerchant();
            ...
            trans.setRedirectDatetime(new Date());
            em.merge(trans);
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doRedirectToMerchant - T1: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
    ...
    public Transactions parsingMIBDompetKuData(Transactions trans,XMLConfiguration xml){
        try {
            ...
            String paymentDate = xml.getString("paymentDate") != null ? xml.getString("paymentDate").trim() : "";
            ...
            String amount = xml.getString("amount") != null ? xml.getString("amount") : "";
            String responseCode = xml.getString("responseCode") != null ? xml.getString("responseCode") : "";
            String bank = xml.getString("bank") != null ? xml.getString("bank") : "";
            
            OneCheckoutLogger.log("::ORDER ID " + orderId + " ::result " + result + " ::MESSAGE " + msg + " ::TRXID " + trxCode);
            if (result != null && result.equalsIgnoreCase("SUCCESS") && responseCode.equals(OneCheckoutErrorMessage.DOMPETKU_SUCCESS_PROCESS.value())) {
    ...
    private String sendMIBRequest(OneCheckoutPaymentRequest paymentRequest,String CoreUrl,MerchantPaymentChannel mpc ,Merchants m ,Transactions trans){
        OneCheckoutLogger.log("Get Redirect Param from Luna Service");
        String response = "";
        try {
            Integer MALLID = mpc.getPaymentChannelCode();
            String SHAREDKEY = m.getMerchantHashPassword();            
            ...
            String msisdn = paymentRequest.getMSISDN() != null ? paymentRequest.getMSISDN().trim() : "";
            String userId = paymentRequest.getUSERID() != null ? paymentRequest.getUSERID().trim() : "" ;
            String desc = "Payment Using DompetKu Clicks";
            ...
            OneCheckoutLogger.log("::" + msisdn +"::" + currency);

            sb.append("SERVICEID=").append(URLEncoder.encode(serviceId, strParam[0])).append(strParam[1]);
            sb.append("ACQUIRERID=").append(URLEncoder.encode(mIDoAcquirerId, strParam[0])).append(strParam[1]);
            ...
            sb.append("TRANSACTIONDATE=").append(URLEncoder.encode(OneCheckoutVerifyFormatData.dompetku_datetimeFormat.format(trans.getIncRequestdatetime()), strParam[0])).append(strParam[1]);
            sb.append("TRANSACTIONNUMBER=").append(URLEncoder.encode(OneCheckoutVerifyFormatData.tenGenerateApprovalCode.format(trans.getTransactionsId()), strParam[0])).append(strParam[1]);
            ...
            sb.append("AUTH1=").append(URLEncoder.encode(msisdn,strParam[0])).append(strParam[1]);
            sb.append("USERID=").append(URLEncoder.encode(userId, strParam[0])).append(strParam[1]);
            String paymentChannelId = mpc.getPaymentChannel().getPaymentChannelId();
//            String ocoId = this.generateOcoId(paymentChannelId);
            sb.append("SERVICETRANSACTIONID=").append(URLEncoder.encode(trans.getOcoId(), strParam[0])).append(strParam[1]);
            
            OneCheckoutLogger.log("::WORDS " + WORDS + " ::BASKET " + basket+" ::MALLID " + MALLID.toString());
            OneCheckoutLogger.log("::CHAINMALLID " + CHAINMALLID+" ::INC DATE TIME " + trans.getIncRequestdatetime());            
            OneCheckoutLogger.log("::TRANS ID " + trans.getTransactionsId() +" ::INVOICE " + invoiceNumber);
            OneCheckoutLogger.log("::CURRENCY " + currency +" ::AMOUNT " + paymentRequest.getAMOUNT());
            OneCheckoutLogger.log("::MSISDN " + msisdn);
    ...
    public OneCheckoutDataHelper doCheckStatus(OneCheckoutDataHelper trxHelper,PaymentChannel paymentChannel){
        ...
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doCheckStatus - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            if (trans != null) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doCheckStatus - Checking status to CORE", (System.currentTimeMillis() - t1));
                trans = super.CheckStatusCOREMIB(trans, Integer.parseInt(mIDoAcquirerId));
                HashMap<String, String> params = super.getData(trans);
                trxHelper.setMessage("VALID");
                params.put("PAYMENTCODE", "");
            ...
            trxHelper.setCheckStatusRequest(statusRequest);
            trxHelper.setMessage("VALID");
            OneCheckoutLogger.log("OneCheckoutV1DompetKuClicksBean.doCheckStatus - T2: %d Finish", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
    ...
    private HashMap<String, String> parseParamRedirect(String inParam,OneCheckoutPaymentRequest paymentRequest) {
        try {
            ...
            for (String param : params) {
                ...
                if (paramName.equals("msisdn")) {
                    paramValue = paymentRequest.getMSISDN();
                }
