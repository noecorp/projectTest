 ...
 */
@Stateless
public class OneCheckoutV1SinarMasVALiteBean extends OneCheckoutChannelBase implements OneCheckoutV1SinarMasVALiteBeanLocal {
    ...
    public OneCheckoutDataHelper doPayment(OneCheckoutDataHelper trxHelper, PaymentChannel pChannel) {
        ...
        try {
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doPayment - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            if (request_good) {
                ...
                redirect.setPageTemplate("sinarmasvalite_input.html");
                ...
                data.put("PAYMENTCHANNEL", OneCheckoutPaymentChannel.SinarMasVALite.value());
...
//                String ocoId = this.generateOcoId(paymentChannelId);
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doPayment - T2: %d Finish", (System.currentTimeMillis() - t1));
    ...
    public OneCheckoutDataHelper doVoid(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
            ...
            voidRequest.setVOIDRESPONSE("FAILED");
             OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doVoid : Void is not permitted");
    ...
    public OneCheckoutDataHelper doInquiryInvoice(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        try {
            ...
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - T0: %d Start process", (System.currentTimeMillis() - t1));
            ...
            String payCode = verifyRequest.getCOMPANYCODE() + verifyRequest.getVAID();

            queryHelper.updateVirtualAccountExpiredTransactions(payCode, merchantCode, acq, mpc.getInvExpiredInMinutes());

            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - querying transaction");
            Transactions trans = null;
            ...
            if (merchant.getMerchantReusablePaycode() != null && merchant.getMerchantReusablePaycode() == Boolean.TRUE) {
                ...
                trxHelper.setPaymentChannel(OneCheckoutPaymentChannel.SinarMasVALite);
                ...
                if (transExist.equalsIgnoreCase("2") || transExist.equalsIgnoreCase("3") || transExist.equalsIgnoreCase("4") || transExist.equalsIgnoreCase("5")) {
                    ...
                    OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - T2: transaction already paid");
                    ...
                    OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - T2: transaction is not found");
                    ...
                    OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice : Inquiry Successfull : %s");
                }
            ...
            if (trans == null) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice : Transaction is null");
            ...
            if (!word.equalsIgnoreCase(verifyRequest.getWORDS())) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice : WORDS doesn't match !");
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - T2: update transaction");
            String ack = createInquiryResponseXML(verifyRequest, trans);

            em.merge(trans);

            //OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - Notify XML : " + ack);

            verifyRequest.setACKNOWLEDGE(ack);

            trxHelper.setVerifyRequest(verifyRequest);

            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInquiryInvoice - T3: %d Finish process", (System.currentTimeMillis() - t1));
    ...
    public OneCheckoutDataHelper doInvokeStatus(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        ...
        try {
            ...
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - T0: %d Start process", (System.currentTimeMillis() - t1));

            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - T1: %d querying transaction", (System.currentTimeMillis() - t1));

            PropertiesConfiguration config = OneCheckoutProperties.getOneCheckoutConfig();

            String payCode = notifyRequest.getCOMPANYCODE() + notifyRequest.getVAID();

            notifyRequest.setSINARMAS_VANUMBER(payCode);
 ...
 //           OneCheckoutNotifyStatusRequest notify = new OneCheckoutNotifyStatusRequest();
            ...
            if (trans == null) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus : Transaction is null");
            ...
            if (!word.equalsIgnoreCase(notifyRequest.getWORDS())) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus : WORDS doesn't match !");
                notifyRequest.setACKNOWLEDGE(OneCheckoutVerifyFormatData.NOTIFYSTATUS_STOP);
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - T2: %d update transaction", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - %s", trans.getDokuHostRefNum());
...
//            notify.setPURCHASECURRENCY(trans.getIncPurchasecurrency());
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - T3: %d start Notify to Merchant", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus : statusNotify : %s", resp);

            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - T4: %d update trx record", (System.currentTimeMillis() - t1));
            em.merge(trans);
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doInvokeStatus - T5: %d Finish process", (System.currentTimeMillis() - t1));
    ...
    public OneCheckoutDataHelper doRedirectToMerchant(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        try {
            ...
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doRedirectToMerchant - T0: %d Start process", (System.currentTimeMillis() - t1));
  ...
  //          redirect.setAMOUNT(redirectRequest.getAMOUNT());
            ...
            em.merge(trans);
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doRedirectToMerchant - T1: %d Finish process", (System.currentTimeMillis() - t1));
...
//            bp.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, config.getString("onecheckout.wsdlurl", "http://localhost:8080/VA/MerchantVAService"));
            ...
            OneCheckoutLogger.log("REGISTER VA REQUEST : " + registrationParamDoc.toString());
            String paymentResponse = port.registerVirtualAccount(registrationParamDoc.toString());
            OneCheckoutLogger.log("RESULT VA REQUEST : " + paymentResponse);
            ...
            OneCheckoutLogger.log("REGISTER VA RESPONSE_CODE : " + vaRegistrationResponse.getResponseCode());
            OneCheckoutLogger.log("REGISTER VA RESPONSE MESSAGE : " + vaRegistrationResponse.getMessage());
    ...
    public OneCheckoutDataHelper doReversal(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        ...
        try {
            ...
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - T0: %d Start process", (System.currentTimeMillis() - t1));

            PropertiesConfiguration config = OneCheckoutProperties.getOneCheckoutConfig();

            String payCode = config.getInt("SINARMAS.INIT.NUMBER") + notifyRequest.getCOMPANYCODE() + OneCheckoutVerifyFormatData.fourDigitMalldId.format(Integer.parseInt(notifyRequest.getMERCHANTCODE())) + OneCheckoutVerifyFormatData.traceNo.format(Integer.parseInt(notifyRequest.getUSERACCOUNT()));
            notifyRequest.setSINARMAS_VANUMBER(payCode);

            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - T1: %d querying transaction", (System.currentTimeMillis() - t1));

            Transactions trans = queryHelper.getPermataVATransactionBy(payCode, notifyRequest.getMERCHANTCODE(), acq, notifyRequest.getSTEP());
            if (trans == null) {

                OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal : Transaction is null");
                notifyRequest.setACKNOWLEDGE(OneCheckoutVerifyFormatData.NOTIFYSTATUS_STOP);
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - T2: %d update transaction", (System.currentTimeMillis() - t1));
            trans.setDokuApprovalCode(notifyRequest.getAPPROVALCODE());
            trans.setDokuIssuerBank(notifyRequest.getBANK());
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - %s", trans.getDokuResultMessage());


            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - T3: %d start Notify to Merchant", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal : statusNotify : %s", resp);
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - T4: %d update trx record", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1SinarMasVALiteBean.doReversal - T5: %d Finish process", (System.currentTimeMillis() - t1));
