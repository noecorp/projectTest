 ...
 */
@Stateless
public class OneCheckoutV1BNIVABean extends OneCheckoutChannelBase implements OneCheckoutV1BNIVABeanLocal {
...
//        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        ...
        try {
            ...
            OneCheckoutLogger.log("OneCheckoutV1BNIVaBean.doPayment - T0: %d Start", (System.currentTimeMillis() - t1));
...
//                redirect.setPageTemplate("alfamartvalite_input.html");
                SimpleDateFormat dtf = new SimpleDateFormat("ddhhmmss");

                int systrace = Integer.parseInt(dtf.format(new Date()));
                String requestRegisterMVA = "";
                String resultRegister = "";
                String urlRegisterVABNI = config.getString("onecheckoutv1.va.bni.register");

                HashMap<String, String> paramComponent = new HashMap<String, String>();
                paramComponent.put("ACQUIRERID", "2009");
                paramComponent.put("TRACENO", systrace + "");
                paramComponent.put("VANUMBER", paymentRequest.getVAinitNumberFormat() + paymentRequest.getPAYCODE());
                paramComponent.put("BILLINGTYPE", "c");
                paramComponent.put("AMOUNT", paymentRequest.getAMOUNT() + "".replace(".", ""));
                paramComponent.put("CUSTOMERNAME", paymentRequest.getNAME());
                paramComponent.put("CUSTOMERPHONE", paymentRequest.getMOBILEPHONE());
                paramComponent.put("CUSTOMEREMAIL", paymentRequest.getEMAIL());

                requestRegisterMVA = super.createParamsHTTP(paramComponent);
                resultRegister = super.postMIP(requestRegisterMVA, urlRegisterVABNI, pChannel);
                if (resultRegister != null || resultRegister.equals("")) {
                    OneCheckoutLogger.log("Response Register VA BNI : \n");
                    XMLConfiguration xmlc = new XMLConfiguration();
                    StringReader sr = new StringReader(resultRegister);
                    xmlc.load(sr);
                    String resultInResponseRegister = xmlc.getString("result");
                    if (!resultInResponseRegister.equalsIgnoreCase("SUCCESS")) {
                        OneCheckoutLogger.log(":: RESPONSE REGISTER IS FAILED ::");
                        trxHelper.setMessage("ERROR");
                        return trxHelper;
                    }
                } else {
                    OneCheckoutLogger.log(":: RESPONSE REGISTER IS NULL ::");
                    //error page
                    trxHelper.setMessage("ERROR");
                    return trxHelper;
                }

                redirect.setPageTemplate("BNIResultPaycode.html");
                ...
                data.put("PAYMENTCHANNEL", OneCheckoutPaymentChannel.BNIVA.value());
...
//                data.put("UA_LOWERCASE", paymentRequest.getUA_LOWERCASE() != null ? paymentRequest.getUA_LOWERCASE() : "");
            ...
            OneCheckoutLogger.log("OneCheckoutV1BNIVaBean.doPayment - T2: %d Finish", (System.currentTimeMillis() - t1));
...
//        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        ...
        OneCheckoutLogger.log("OneCheckoutV1BNIVaBean.doVoid : Void is not permitted");
...
//        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1BNIVaBean.doRedirectToMerchant - T0: %d Start process", (System.currentTimeMillis() - t1));
            Merchants m = trxHelper.getMerchant();
            ...
            em.merge(trans);
            OneCheckoutLogger.log("OneCheckoutV1BNIVaBean.doRedirectToMerchant - T1: %d Register DOKUWallet", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1BNIVaBean.doRedirectToMerchant - T1: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
