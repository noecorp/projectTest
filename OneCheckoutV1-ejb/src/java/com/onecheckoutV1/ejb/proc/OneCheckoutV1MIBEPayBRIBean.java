 ...
 */
@Stateless
public class OneCheckoutV1MIBEPayBRIBean extends OneCheckoutChannelBase implements OneCheckoutV1MIBEPayBRIBeanLocal {
    ...
    public OneCheckoutDataHelper doMIPPayment(OneCheckoutDataHelper oneCheckoutDataHelper, PaymentChannel paymentChannel) {
        ...
        try {
            ...
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doMIPPayment - T0: %d Start", (System.currentTimeMillis() - t1));
                    ...
                    try {
                        ...
                        if (xml.getRootElementName().equals("MIBPaymentResponse") && xml.getString("result").equals("REDIRECT")) {
                            ...
                            String xmlMessage = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
                                    ...
                                    "</PAYMENT_STATUS>";
                            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doMIPPayment - RESPONSE TO MERCHANT : " + xmlMessage);
            ...
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doMIPPayment - T2: %d Finish process", (System.currentTimeMillis() - t1));
    ...
    public OneCheckoutDataHelper doPayment(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        ...
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doPayment - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doPayment - T2: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
    ...
    private String preRedirect(OneCheckoutPaymentRequest paymentRequest, String urlRedirect, MerchantPaymentChannel mpc, Merchants merchants, String ocoId) {
        ...
        try {
            ...
            String desc = "Payment using EPay BRI";
            int pos = basket.indexOf(",");
            if (pos > 0) {
                desc = basket.substring(0, pos);
            }
            StringBuilder sb = new StringBuilder();
            sb.append("SERVICEID=").append(URLEncoder.encode("1", "UTF-8")).append("&");
            sb.append("ACQUIRERID=").append(URLEncoder.encode("151", "UTF-8")).append("&");
...
//            String ocoId = this.generateOcoId(paymentChannelId);
            sb.append("OCOID=").append(URLEncoder.encode(ocoId, "UTF-8")).append("&");
            ...
            Logger.getLogger(OneCheckoutV1MIBEPayBRIBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return response;
    }
    ...
    public OneCheckoutDataHelper doInvokeStatus(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - T0: %d Start process", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - T1: %d querying transaction", (System.currentTimeMillis() - t1));
            Transactions trans = trxHelper.getTransactions();
            if (trans == null) {
                OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus : Transaction is null");
            ...
            if (!word.equalsIgnoreCase(notifyRequest.getWORDS())) {
                OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus : WORDS doesn't match !");
                notifyRequest.setACKNOWLEDGE(OneCheckoutVerifyFormatData.NOTIFYSTATUS_STOP);
                trxHelper.setNotifyRequest(notifyRequest);
                return trxHelper;
            }

            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - T2: %d update transaction", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - %s", trans.getDokuResultMessage());
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - T3: %d start Notify to Merchant", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus : statusNotify : %s", resp);
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - T4: %d update trx record", (System.currentTimeMillis() - t1));
            em.merge(trans);

            notifyRequest.setACKNOWLEDGE(resp);
            trxHelper.setNotifyRequest(notifyRequest);
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doInvokeStatus - T5: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
    ...
    public OneCheckoutDataHelper doRedirectToMerchant(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doRedirectToMerchant - T0: %d Start process", (System.currentTimeMillis() - t1));
            Merchants m = trxHelper.getMerchant();
            ...
            trans.setRedirectDatetime(new Date());
            em.merge(trans);
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doRedirectToMerchant - T1: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
    ...
    public OneCheckoutDataHelper doCheckStatus(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        ...
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doCheckStatus - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            if (trans != null) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doCheckStatus - Checking status to CORE", (System.currentTimeMillis() - t1));
                trans = super.CheckStatusCOREMIB(trans, 151);
                HashMap<String, String> params = super.getData(trans);
                trxHelper.setMessage("VALID");
                params.put("PAYMENTCODE", "");
            ...
            trxHelper.setCheckStatusRequest(statusRequest);
            trxHelper.setMessage("VALID");
            OneCheckoutLogger.log("OneCheckoutV1MIBEPayBRIBean.doCheckStatus - T2: %d Finish", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
