...
@Stateless
public class OneCheckoutV1CIMBClicksBean extends OneCheckoutChannelBase implements OneCheckoutV1CIMBClicksBeanLocal {
    ...
    public OneCheckoutDataHelper doPayment(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        ...
        try {
            ...
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doPayment - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            if (request_good) {
                ...
                PaymentChannel pc = queryHelper.getPaymentChannelByChannel(OneCheckoutPaymentChannel.CIMBClicks);
                    ...
                    if (!xmls.equals("")){
                        ...
                        try {
                            ...
                            if (xml.getRootElementName().equals("MIBPaymentResponse") && xml.getString("result").equals("REDIRECT")){
                                ...
                                /*
                                String GOBACK = config.getString("CIMBCLICKS.GOBACK");
                                String GOBACKAUTH = config.getString("CIMBCLICKS.GOBACK.AUTH");
                                ...
                                GOBACK = GOBACK
                                        .replace("{1}", URLEncoder.encode(NUMBERID.toString(), "UTF-8"))
                                        .replace("{2}", URLEncoder.encode(CHECKPAY, "UTF-8"))
                                        .replace("{3}", URLEncoder.encode(OneCheckoutVerifyFormatData.sdf.format(trans.getIncAmount().doubleValue()), "UTF-8"));
            ...
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doPayment - T2: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
    ...
    public OneCheckoutDataHelper doInvokeStatus(OneCheckoutDataHelper trxHelper, PaymentChannel paymentChannel) {
        ...
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus - T0: %d Start process", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus - T1: %d querying transaction", (System.currentTimeMillis() - t1));
            Transactions trans = queryHelper.getNotifyTransactionBy(invoiceNo, sessionId, paymentChannel);
            if (trans == null) {
                OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus : Transaction is null");
            ...
            if (!word.equalsIgnoreCase(notifyRequest.getWORDS())) {
                OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus : WORDS doesn't match !");
                notifyRequest.setACKNOWLEDGE(OneCheckoutVerifyFormatData.NOTIFYSTATUS_STOP);
                trxHelper.setNotifyRequest(notifyRequest);
                return trxHelper;
            }
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus - T2: %d update transaction", (System.currentTimeMillis() - t1));
           ...
           //     trans.setEduStatus(OneCheckoutDFSStatus.NA.value());
            ...
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus - T3: %d start Notify to Merchant", (System.currentTimeMillis() - t1));
            ...
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus - T4: %d update trx record", (System.currentTimeMillis() - t1));
            em.merge(trans);
            notifyRequest.setACKNOWLEDGE(resp);
            trxHelper.setNotifyRequest(notifyRequest);
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doInvokeStatus - T5: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
    ...
    public OneCheckoutDataHelper doRedirectToMerchant(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doRedirectToMerchant - T0: %d Start process", (System.currentTimeMillis() - t1));
            Merchants m = trxHelper.getMerchant();
            ...
            trans.setRedirectDatetime(new Date());
            em.merge(trans);
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doRedirectToMerchant - T1: %d Finish process", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
    ...
    public OneCheckoutDataHelper doCheckStatus(OneCheckoutDataHelper trxHelper, PaymentChannel acq) {
       OneCheckoutCheckStatusData statusRequest = trxHelper.getCheckStatusRequest();
        try {
            long t1 = System.currentTimeMillis();
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doCheckStatus - T0: %d Start", (System.currentTimeMillis() - t1));
            ...
            if (trans != null) {
                ...
                OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doCheckStatus - Checking status to CORE", (System.currentTimeMillis() - t1));
                trans = super.CheckStatusCOREMIB(trans, 6);
                HashMap<String, String> params =  super.getData(trans);
                trxHelper.setMessage("VALID");
                params.put("PAYMENTCODE","");
            ...
            trxHelper.setCheckStatusRequest(statusRequest);
            trxHelper.setMessage("VALID");
            OneCheckoutLogger.log("OneCheckoutV1CIMBClicksBean.doCheckStatus - T2: %d Finish", (System.currentTimeMillis() - t1));
            return trxHelper;
        } catch (Throwable th) {
            th.printStackTrace();
            trxHelper.setMessage(th.getMessage());
    ...
    public String preRedirect(OneCheckoutPaymentRequest paymentRequest, String urlRedirect, MerchantPaymentChannel mpc, Merchants merchants, String ocoId){
        OneCheckoutLogger.log("============ preRedirect | Get Redirect Param From MIB ===========");
        String response = "";
        try {
...
//            String ocoId = this.generateOcoId(paymentChannelId);
            ...
            sb.append("ACQUIRERID=").append(URLEncoder.encode("1003", "UTF-8")).append("&");
